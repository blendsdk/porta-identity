    ...
    projects: [
      {
        test: {
          ...
          environment: 'node',
          testTimeout: 10_000,
          // Comprehensive env block so test files that transitively import
          // src/config/index.ts (e.g. via src/lib/database.ts or src/lib/signing-keys.ts)
          // get valid config values — even in CI where there is no .env file.
          env: {
