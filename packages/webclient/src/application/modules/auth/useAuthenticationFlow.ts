...
import { getCurrentLocale, useObjectState } from "@blendsdk/react";
...
export const useAuthenticationFlow = () => {
    ...
    const form = useFormik<IAuthenticationDialogModel>({
        ...
        validate: (values) => {
            if (state.resp === RESP_FORGOT_PASSWORD) {
                const val = {
            ...
            } else if (state.resp === RESP_ACCOUNT) {
                const val = {
                    username: validateData(values.username, (data) => {
                        yup.string().required("username_is_required").validateSync(data);
                    }),
        ...
        onSubmit: (values) => {
            ...
            if (state.resp === RESP_ACCOUNT || state.curState === RESP_ACCOUNT) {
                ...
                ApplicationApi.authorization
                    ...
                    .then(({ data }) => {
                        // Empty the password field if we need to change the password
                        // This will force the user to fill in the password again
                        if (data.resp === RESP_CHANGE_PASSWORD) {
                        ...
                        setState({
                            ...
                            curState:
                                data.resp !== state.curState && data.resp !== INVALID_PWD ? data.resp : state.curState,
                            ...data
                        });
                    })
                ...
                ApplicationApi.authorization
                    ...
                    .then(({ data }) => {
                        setState({
                            fetching: isFinalize(data.resp),
                            curState:
                                data.resp !== state.curState && data.resp !== INVALID_PWD_MATCH
                                    ? data.resp
                                    : state.curState,
                            ...data
                        });
                    ...
                    .catch(catchSystemError);
            } else if (state.resp === RESP_FORGOT_PASSWORD || state.curState === RESP_FORGOT_PASSWORD) {
                setState({ fetching: true });
                ApplicationApi.authorization
                    .checkSetFlow({
        ...
        ApplicationApi.authorization
            ...
            .then(({ data }) => {
                setState({
                    fetching: isFinalize(data.resp),
                    curState:
                        data.resp === RESP_FORGOT_PASSWORD || data.resp === RESP_FORGOT_PASSWORD
                            ? data.resp
                            : state.curState,
                    ...data
                });
