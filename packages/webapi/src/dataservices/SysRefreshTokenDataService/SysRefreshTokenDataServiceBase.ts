...
export abstract class SysRefreshTokenDataServiceBase extends DataService<PostgreSQLExecutionContext> {
	...
	public async findSysRefreshTokenById(
		...
		const ctx = await this.getContext();
		const result = await ctx.executeQuery<ISysRefreshToken, ISysRefreshTokenDataServiceFindSysRefreshTokenByIdParams>(
			`SELECT * FROM sys_refresh_token WHERE id = :id`,
			params,
			{ single: true }
		);
		return result.data;
