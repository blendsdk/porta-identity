...
export abstract class SysSessionDataServiceBase extends DataService<PostgreSQLExecutionContext> {
	...
	public async findSysSessionByUserIdAndClientId(
		...
		const ctx = await this.getContext();
		const result = await ctx.executeQuery<ISysSession, ISysSessionDataServiceFindSysSessionByUserIdAndClientIdParams>(
			`SELECT * FROM sys_session WHERE user_id = :user_id AND client_id = :client_id`,
			params,
			{ single: true }
		);
		return result.data;
