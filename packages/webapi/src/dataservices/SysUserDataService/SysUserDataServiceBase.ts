...
export abstract class SysUserDataServiceBase extends DataService<PostgreSQLExecutionContext> {
    ...
    /**
     * @param {ISysUserDataServiceFindMfaByUserIdParams}
     * @returns {ISysUserMfaView[]}
     * @memberof SysUserDataServiceBase
     */
    public async findMfaByUserId(params: ISysUserDataServiceFindMfaByUserIdParams): Promise<ISysUserMfaView[]> {
        const ctx = await this.getContext();
        const result = await ctx.executeQuery<ISysUserMfaView[], ISysUserDataServiceFindMfaByUserIdParams>(
            `SELECT * FROM sys_user_mfa_view WHERE user_id = :user_id::uuid`,
            params,
            { single: false }
        );
        return result.data;
     ...
     * @memberof SysUserDataServiceBase
     */
    public async findByUsernameNonService(
        params: ISysUserDataServiceFindByUsernameNonServiceParams
    ): Promise<ISysUser> {
        const ctx = await this.getContext();
