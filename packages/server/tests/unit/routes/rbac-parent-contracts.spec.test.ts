...
describe('committed RBAC mutation results', () => {
  // Ordinary application roles remain removable from the bootstrap user. The service applies the
  // survivor rule only when the exact canonical super-admin assignment is part of the request.
  it('delegates ordinary role removal for the bootstrap administrator to the service', async () => {
    mocks.guardSuperAdmin.mockRejectedValue(new Error('route guard must not run'));
    mocks.userRoleRemove.mockResolvedValue({ reauthenticationRequired: false });
    const ctx = context({
    ...
    expect(mocks.guardSuperAdmin).not.toHaveBeenCalled();
    expect(mocks.userRoleRemove).toHaveBeenCalledWith(ORG_ID, USER_ID, [ROLE_ID], ACTOR_ID);
  });
