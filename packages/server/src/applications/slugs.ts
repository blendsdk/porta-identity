/**
 * Slug generation and validation utilities for applications.
 *
 * Application slugs are URL-safe identifiers used to reference
 * applications in API paths and configuration. They must be
 * 3–100 characters, lowercase alphanumeric with hyphens,
 * and cannot be a reserved system word.
 *
 * This module provides:
 * - `generateSlug(name)` — Derive a slug from an application name
 * - `validateSlug(slug)` — Check format and reserved-word rules
 * - `RESERVED_SLUGS` — The set of words that cannot be used as application slugs
 *
 * Note: Application reserved words differ from organization reserved words
 * because app slugs appear in different contexts (e.g., app references in
 * client metadata, module namespacing).
 */
...
/**
 * Reserved slugs that cannot be used as application slugs.
 * These conflict with system concepts, internal naming, or
 * could cause confusion with infrastructure functionality.
 */
...
/**
 * Generate a URL-safe slug from an application name.
 *
 ...
 *
 * @param name - Application name to derive a slug from
