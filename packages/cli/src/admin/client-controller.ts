...
export interface AdminClientControllerOptions {
  ...
  readonly presentSecret?: (
    secret: AdminClientSecretPresentation,
    signal: AbortSignal,
  ) => Promise<void>;
...
export interface AdminClientController {
  ...
  /** Confirms and permanently deletes one client at most once. */
  readonly delete: (
    clientId: string,
    confirm: (signal: AbortSignal) => Promise<boolean>,
  ) => Promise<void>;
...
type ClientCapability =
  'canCreateClients' | 'canUpdateClients' | 'canDeleteClients' | 'canRevokeClientSecrets';
...
export function createAdminClientController(
  ...
  const cancel = (reconcileDispatchedMutation = false): void => {
    const uncertainOwner =
      reconcileDispatchedMutation && mutationDispatched ? organizationId : undefined;
    generation += 1;
    operation?.abort();
    operation = undefined;
  ...
  const presentationValue = (
    value: ClientMutationValue,
  ): AdminClientSecretPresentation | undefined => {
    if ('client' in value) {
    ...
    const retained = projection;
    if (!retained || retained.kind === 'list' || retained.client.id !== value.clientId)
      return undefined;
    return {
      ...value,
    ...
    if (result.kind === 'failure') {
      if (reloadOnFailure) {
        const selectedClientId =
          projection && projection.kind !== 'list' ? projection.client.id : undefined;
        operation = undefined;
        if (selectedClientId) await select(selectedClientId);
        else await load();
      }
      if (organizationId === owner) publishFailure(owner, result);
      mutationDispatched = false;
      return;
    }
  ...
  const mutate = async (
    ...
    try {
      if (
        confirm &&
        (!(await confirm(controller.signal)) || !owns(capturedGeneration, owner, controller))
      )
        return;
      const current = options.readState();
      const operations = options.readOperations();
      ...
      const completion = finishMutation(result, owner, controller, reloadAfter, reloadOnFailure);
      handedOff = true;
      return completion;
    } finally {
  ...
  return {
    ...
    update: (clientId, input, etag) =>
      mutate(
        'canUpdateClients',
        (operations, signal, owner) =>
          operations.update?.(owner, clientId, input, etag, signal) ??
          Promise.resolve({ kind: 'cancelled' }),
        () => select(clientId),
        undefined,
        () => ownsMutableTarget(clientId, 'update'),
      ),
    activate: (clientId, confirm) =>
      mutate(
        'canUpdateClients',
        (operations, signal, owner) =>
          operations.activate?.(owner, clientId, signal) ?? Promise.resolve({ kind: 'cancelled' }),
        () => select(clientId),
        confirm,
        () => ownsMutableTarget(clientId, 'lifecycle'),
      ),
    deactivate: (clientId, confirm) =>
      mutate(
        'canUpdateClients',
        (operations, signal, owner) =>
          operations.deactivate?.(owner, clientId, signal) ??
          Promise.resolve({ kind: 'cancelled' }),
        () => select(clientId),
        confirm,
        () => ownsMutableTarget(clientId, 'lifecycle'),
      ),
    delete: (clientId, confirm) =>
      mutate(
        'canDeleteClients',
        (operations, signal, owner) =>
          operations.delete?.(owner, clientId, signal) ?? Promise.resolve({ kind: 'cancelled' }),
        load,
        confirm,
        () => ownsMutableTarget(clientId, 'lifecycle'),
        true,
    ...
    loadSecrets,
    generateSecret: (clientId, input) =>
      mutate(
        'canUpdateClients',
        (operations, signal, owner) =>
          operations.generateSecret?.(owner, clientId, input, signal) ??
          Promise.resolve({ kind: 'cancelled' }),
        () => loadSecrets(clientId),
        undefined,
        () => ownsMutableTarget(clientId, 'generate'),
      ),
    revokeSecret: (clientId, secretId, confirm) =>
      mutate(
        'canRevokeClientSecrets',
        (operations, signal, owner) =>
          operations.revokeSecret?.(owner, clientId, secretId, signal) ??
          Promise.resolve({ kind: 'cancelled' }),
        () => loadSecrets(clientId),
        confirm,
        () => ownsMutableTarget(clientId, 'revoke-secret', secretId),
      ),
