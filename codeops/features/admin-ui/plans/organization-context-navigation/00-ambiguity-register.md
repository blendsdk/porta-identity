...
|  17 | Packed cleanup (runtime)     | How can exact SDK cleanup avoid the existing server's UUID-first slug lookup failure without changing server or SDK scope?                            | A: verify the unique slug/name nonce, then destroy that exact projection by its UUID / B: fix the server lookup outside this plan                                                                | AI selected A under delegated auto-design authority               | ✅ Resolved |
|  18 | Scope dependency (runtime)   | How can packed SDK cleanup complete when the existing server cascade-count query references an organization column absent from the application table? | A: authorize the minimal server correctness fix and required verification / B: bypass the Admin API with direct playground SQL / C: defer the packed cleanup task                                | User authorized Option A                                          | ✅ Resolved |
...
**AR-17 (runtime):** Authority: AI — delegated by `--auto-design`. Eligibility: test cleanup
mechanism within the approved exact-resource boundary. Objective: remove only the organization
created by the packed journey without adding server or SDK work. Decision: the packed SDK child
lists by the unique high-entropy slug, proves the matching projection carries the nonce-bearing
name, and passes that verified projection's UUID to `organizations.destroy()`. Evidence: the live
playground returned `503 admin_mutation_unavailable`, while PostgreSQL recorded the server trying
`SELECT * FROM organizations WHERE id = $1` with the slug. Passing the verified UUID avoids that
lookup failure and reaches the server's deletion path, where AR-18 records a separate blocker.
Rejected alternative: fixing the server's UUID-first lookup is outside the approved plan and would
require server verification. Strongest counterargument: passing the slug more literally
matches the original cleanup wording, but it cannot reach deletion on the current server and does
not improve resource ownership. Confidence: High. Hardening: the destructive boundary remains
narrower than the input—the child deletes only the UUID read from the sole exact slug/name match
and verifies both its absence and preservation of every unrelated ID. Policy version: 1. Root
invocation ID: `ad-20260828-admin-ui-rd02`. Reopen trigger: the server accepts organization slugs
through the destroy endpoint or the SDK stops returning a stable organization UUID.

**AR-18 (runtime):** Authority: user-owned because resolving it expands the approved UI-only scope.
The live packed journey authenticates, cancels the initial chooser, opens Who am I, switches
organizations, creates a high-entropy organization, and automatically selects it. Exact cleanup by
the verified UUID then returns fixed `503 admin_mutation_unavailable`. PostgreSQL records
`column "organization_id" does not exist` from `getCascadeCounts()`, which queries
`applications.organization_id` even though applications are global in the current schema.
Decision: the user authorized Option A on 2026-08-28, limited to correcting that existing
repository query and running the server verification required by project policy. Option B is
rejected because direct SQL would bypass the Admin API's authorization, transaction, and audit
behavior. Option C leaves ST-33–ST-34 incomplete. Confidence: High. Reopen trigger: applications or
their global role, permission, and claim definitions become organization-owned in the schema.
