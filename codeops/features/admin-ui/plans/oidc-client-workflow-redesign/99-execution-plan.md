...
- [ ] 1.3.1 Add internal route and SDK serialization coverage — `packages/server/tests/unit/routes/client-secret-expiry.impl.test.ts`, `packages/sdk/tests/domains/client-secret-expiry.impl.test.ts`
...
- [ ] 3.2.1 Replace the long detail block with one responsive `ListBox`, local section state, GroupBox regions, authoritative Overview summaries, a small name editor, and separate Back navigation — `packages/cli/src/admin/client-workspace.ts`, `packages/cli/src/admin/client-dialogs.ts`
