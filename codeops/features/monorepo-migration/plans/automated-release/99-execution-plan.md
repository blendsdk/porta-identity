...
- [x] 1.1.1 [spec-author] Rewrite `repo-tests/monorepo/release.spec.test.mjs` and `repo-tests/monorepo/release-hardening.spec.test.mjs` for ST-1..ST-7 ✅ (completed: 2026-09-25 10:49; contract specs rewritten)
- [x] 1.1.2 Run `yarn test:structure` and verify the new assertions FAIL against the current workflow (red phase) ✅ (completed: 2026-09-25 10:49; red: 2 release specs fail)
...
- [x] 2.1.1 Rebuild `.github/workflows/release.yml`: `workflow_dispatch` (bump/dry_run), gate, bump, preflight, publish, explicit commit/tag push, GitHub Release, Docker dispatch, develop sync, idempotency ✅ (completed: 2026-09-25 10:51)
- [x] 2.1.2 Update `package.json` `release:publish` to drop `--git-push` (keep `--provenance`); keep `release:prepare` ✅ (completed: 2026-09-25 10:51; already correct; release:publish has --provenance and no --git-push)
- [x] 2.1.3 Run `yarn test:structure` and verify it PASSES (green phase) ✅ (completed: 2026-09-25 10:51; green: structure 128/128)
...
- [x] 4.1.1 Merge `feat/automated-release` into `develop` (PR, CI green) ✅ (completed: 2026-09-25 11:28; PR #130 merged)
- [x] 4.1.2 Merge `develop` into `main` (deploys the new workflow; no automatic release fires because the workflow no longer declares `workflow_run`) ✅ (completed: 2026-09-25 11:28; PR #131 merged; main fb11f971)
