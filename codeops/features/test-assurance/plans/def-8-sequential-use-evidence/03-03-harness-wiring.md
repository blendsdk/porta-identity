...
| production-security block (after the functional block)                | Lifecycle `reset`, then run `humanAuthRecoverySpecificationFiles` with `{...environmentForManifest(active.lease.manifest), PORTA_ASSURANCE_PROJECT:'security', PORTA_ASSURANCE_HUMAN_AUTH_ADAPTER:'live'}`, and fail fast on a non-zero exit |
