...
| ST-49 | Send concurrent duplicate consumes through the real public boundary and inspect durable state after both settle                             | Exactly one durable success and one rejected competitor; no reusable intermediate value remains                                                               | R5.7, R5.12                         |
| ST-50 | Invoke the real Redis/PostgreSQL conditional-consume boundary concurrently for each supported artifact                                      | Exactly one durable consumed transition is recorded; the competing consume cannot create another effect                                                       | R5.7, R5.12, Single-Use Consistency |
| ST-51 | Discard one committed client response, retry from durable state, gracefully restart owned Porta, and replay                                 | Both retries are rejected as replay; the process identity changes and no duplicate token/session/account effect occurs                                        | R5.7, R5.12, Single-Use Consistency |
