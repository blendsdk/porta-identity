...
| `yarn assurance:fault --fault <fault-id> --claim <claim-id> --sentinel <sentinel-id>` or `--recover-control <run-uuid>`       | clean baseline; registered tuple; Docker; disposable-worktree support                    |                                                                                                          3,600 s | `fault/<fault>/<claim>/<sentinel>/` | owns worktree/build/image/stack; tenant/admin operator evidence uses the defensive control-sensitivity vocabulary; primary tree immutable; signal cleanup/recovery exact                         |
