...
| ID    | Scenario                                                                                                                                 | Exact expected behavior                                                                                                                                       | Source                                 |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| ST-42 | Existing/absent identity login and recovery requests                                                                                     | Status/body/header match; the timing-distribution edge remains blocked under DEF-7 because no independent pre-measurement contract was approved              | R5.6, AC7                              |
...
| ST-62  | Umbrella edge for the approved immutable bulk/import/export duplicate, collision, provenance, rollback, partial-result, sensitivity, and tenant catalog | Product-remediation administrative-data specifications and production-backed observations execute the approved catalog | R5.9, R4.12 |
...
| ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------: | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `yarn assurance:test --select <registered-suite\|ST-ID\|internal-test-path>`                                                  | frozen root install; definitions valid; case-declared services healthy                   | 120 s; `fixture-ontology`: 900 s; `fixtures-all`: 900 s per child | `test/`                             | Node/tsx internal runner only; no owned stack unless declared; SIGINT/SIGTERM use common cleanup                                                                                                 |
...
| `yarn assurance:fault --fault <fault-id> --claim <claim-id> --sentinel <sentinel-id>`                                       | clean baseline; registered tuple; Docker; disposable-worktree support                    |                                                                                                          3,600 s | `fault/<fault>/<claim>/<sentinel>/` | owns its curated-fault worktree; primary tree immutable; signal cleanup/recovery exact                                                                                                           |
| `yarn assurance:mutation --select bounded-pilot` or `--recover <run-uuid>`                                                   | clean baseline; exact StrykerJS packages; disposable-worktree support; allowlisted targets |                                                                                                            900 s | `mutation/bounded-pilot/`          | owns one detached worktree and runtime directory; primary tree immutable; signal cleanup/recovery exact; retains only count evidence                                                             |
| `yarn assurance:control-check --check <registered-check>` or `--recover <run-uuid>`                                         | clean baseline; registered check; Docker; disposable-worktree support                    |                                                                                                          3,600 s | `control-check/<check>/`            | owns one local source variant and lifecycle stack; no arbitrary path, command, endpoint, or external target; signal cleanup/recovery exact                                                       |
| `yarn assurance:compat --select <ST-69\|ST-70\|ST-71\|ST-72\|ST-73\|tenant-admin\|protocol\|compatibility>`                    | built local SDK/CLI archives; clean consumer; isolated temporary `HOME`; healthy harness |                                                                                                          1,800 s | `compat/<selector>/`                | owns consumer/home/clients; real credential fingerprint unchanged on every outcome                                                                                                               |
| `yarn assurance:report --run <run-uuid> --coverage-run <coverage-run-uuid>`                                                    | sanitized completed/incomplete manifest and matching provenance-bound security coverage observation exist |                                                                                                            120 s | `summary/`                          | reads the two exact owned runs only; applies local coverage ratchets; pre-write redaction; no service ownership                                                                                   |
...
|   50 | `assurance-invalid`   | A validated local variant cannot build/start or its registered tuple/signature/check result is invalid; a not-detected check exits 21                  |
