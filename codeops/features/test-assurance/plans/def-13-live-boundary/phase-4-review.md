...
| ID  | Resolution                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1  | Removed the unobservable forwarded assertions from the P1 lane only. An initial attempt removed them from the shared catalog, which silently closed DEF-14's known gaps (`incomplete` went 3 to 0); that was reverted and replaced with optional `p1IndependentStateObservations` / `p1ProhibitedSideEffects` fields. The production-exposure lane keeps its three incomplete gaps; the P1 lane asserts only the observed reflected-value fact. |
...
Verified: `yarn test:structure` and the server `admin-auth` unit suite (13/13) pass; the operational
harness run `729dfdb8-88b5-48c2-808b-b7da6ebfcc67` preserves the accepted production-exposure
`passed=3 incomplete=3` exit 40 and passes the P1 live suite. The remaining M2 gap is an
oracle-level limitation, not an adapter defect.
