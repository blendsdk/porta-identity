...
test:structure` before each commit; the full production-security evidence run once at the end.
...
| 2     | `npx tsx --test test-harness/assurance/tests/p1-live-boundaries.spec.test.ts` (raw lane)                      |
| 3     | `npx tsx --test test-harness/assurance/tests/p1-live-boundaries.spec.test.ts` (admin lane)                    |
| 4     | `yarn assurance:harness --project security --profile production-security` once; `yarn test:structure`         |

---

## Phase 0: Case observability audit (complete)

- [x] 0.1 Enumerate the raw and admin cases with paths, profiles, and required fields; classify the emitting surface. (completed 2026-09-21 11:25; 33 cases: 29 admin-api, 3 /health, 1 /alpha/authorize; no product change required)
...
- [ ] 2.1 Implement `observeValidationCase` for the 11 operational raw cases, reusing the production-exposure live adapter internals for `st53` and shared request materialization.
- [ ] 2.2 Correlate each raw request to its decision (request-id header, or bounded window plus reason code for transport rejections).
...
## Phase 4: Evidence, credit, and closeout

- [ ] 4.1 Register the P1 live selector and wire correlated-log credit, replacing the hard-coded false gap.
- [ ] 4.2 Run the production-security evidence once and record provenance.
