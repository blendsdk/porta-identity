...
| DEF-16           | Bounded `/health` and `/ready` probes, pool-error survival, reconnect regression test; focused suite green. Live refresh: production-security run `c9716d5b-fcc6-4461-b1b4-cabded7b6458` admitted all three `st56-production-security-*` dependency cases as passed (`503`/`200`) with no restart requirement |
...
1. **DEF-13 residual** — the administrative-data live adapter is delivered in PR #104; merge it to
   complete the correlated-log credit.
2. **DEF-14 then DEF-8** — both reuse the existing human-auth and production-exposure scaffolding.
3. **DEF-3, DEF-6, DEF-22** — protocol and command-signal campaigns.
4. **DEF-5, DEF-11, DEF-17** — source-variation sensitivity registries.
5. **DEF-21 and DEF-23** — product and security-authority rulings that gate "fully production
   ready".
