...
| DEF-13 | Correlated P1 rejection events                       | Open (harness review)       | Resolve the Phase 4 review C1–C3 fixes (real forwarded-context observers, forbidden-field scanning, non-empty cursor) and the M3/M4 claim rulings                   | None (harness)                           | `test-harness/assurance/p1/live-adapter.ts`; `p1-live-boundary`                                                          |
...
2. **DEF-13 review fixes** (C1–C3) plus M3/M4 claim rulings — the adapter passes the oracle, but the
   Phase 4 review found assertions that do not verify the named properties; then DEF-14
   forwarding-context observers.
