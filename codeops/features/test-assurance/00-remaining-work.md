...
| DEF-13 | Correlated P1 rejection events                       | Partial                     | Emit one audit event plus a target digest for administrative permission denials, then award external correlated-log credit                                          | Product (small)                          | `packages/server/src/middleware/require-permission.ts`; `test-harness/assurance/tests/p1-live-adapter.ts`; `p1-specs`    |
