...
// Alpha and bravo fixture ownership, redirects, origins, scopes, and credentials are disjoint so
// a cross-tenant result cannot be explained by ambiguous shared fixture identity.
test('should keep every alpha and bravo tenant identity disjoint', async () => {
  ...
  assert.deepEqual(
    intersection(
      alpha.clients.flatMap((client) => client.scopes),
      bravo.clients.flatMap((client) => client.scopes),
    ),
    [],
  );
