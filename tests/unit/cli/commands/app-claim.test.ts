...
import { appClaimCommand } from '../../../../src/cli/commands/app-claim.js';
...
function getHandlers() {
  ...
  (appClaimCommand.builder as (y: typeof fakeYargs) => typeof fakeYargs)(fakeYargs);
  return handlers;
}
...
describe('CLI App Claim Command', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  describe('claim create', () => {
    it('should resolve app and POST to claims endpoint', async () => {
      mockClient.post.mockResolvedValue({ status: 201, data: { data: fakeClaim } });
      ...
      expect(mockClient.post).toHaveBeenCalledWith(`/api/admin/applications/${APP_UUID}/claims`, {
        claimName: 'department',
        claimType: 'string',
        description: 'User department',
      });
  ...
  describe('claim list', () => {
    it('should resolve app and GET claim definitions', async () => {
      mockClient.get.mockResolvedValue({ status: 200, data: { data: [fakeClaim] } });

      const handlers = getHandlers();
      await handlers['list'](createArgv({ app: APP_UUID }));

      expect(mockResolveApp).toHaveBeenCalledWith(mockClient, APP_UUID);
      expect(mockClient.get).toHaveBeenCalledWith(`/api/admin/applications/${APP_UUID}/claims`);
      expect(printTable).toHaveBeenCalled();
    });

    it('should warn when no claim definitions found', async () => {
      mockClient.get.mockResolvedValue({ status: 200, data: { data: [] } });

      const handlers = getHandlers();
      await handlers['list'](createArgv({ app: APP_UUID }));

      expect(warn).toHaveBeenCalledWith('No claim definitions found');
    });
  });
  ...
  describe('claim update', () => {
    it('should PUT claim with updated description', async () => {
      mockClient.put.mockResolvedValue({ status: 200, data: { data: { ...fakeClaim, description: 'Updated' } } });

      const handlers = getHandlers();
      await handlers['update'](createArgv({
        app: APP_UUID,
        'claim-id': fakeClaim.id,
        description: 'Updated',
      }));

      expect(mockResolveApp).toHaveBeenCalledWith(mockClient, APP_UUID);
      expect(mockClient.put).toHaveBeenCalledWith(
        `/api/admin/applications/${APP_UUID}/claims/${fakeClaim.id}`,
        { description: 'Updated' },
      );
      expect(success).toHaveBeenCalledWith(expect.stringContaining('department'));
    });
  ...
  describe('claim delete', () => {
    it('should skip with dry-run', async () => {
      const handlers = getHandlers();
      await handlers['delete'](createArgv({ app: APP_UUID, 'claim-id': fakeClaim.id, 'dry-run': true }));
    ...
    it('should cancel when confirm rejects', async () => {
      ...
      const handlers = getHandlers();
      await handlers['delete'](createArgv({ app: APP_UUID, 'claim-id': fakeClaim.id }));

      expect(warn).toHaveBeenCalledWith('Operation cancelled');
      expect(mockClient.delete).not.toHaveBeenCalled();
    });

    it('should DELETE claim definition on confirmation', async () => {
      mockClient.delete.mockResolvedValue({ status: 204, data: {} });

      const handlers = getHandlers();
      await handlers['delete'](createArgv({ app: APP_UUID, 'claim-id': fakeClaim.id, force: true }));

      expect(mockResolveApp).toHaveBeenCalledWith(mockClient, APP_UUID);
      expect(mockClient.delete).toHaveBeenCalledWith(
        `/api/admin/applications/${APP_UUID}/claims/${fakeClaim.id}`,
      );
      expect(success).toHaveBeenCalledWith(expect.stringContaining(fakeClaim.id));
    });
  });
  ...
  describe('metadata', () => {
    it('should export command name "claim"', () => {
      expect(appClaimCommand.command).toBe('claim');
    });

    it('should have a describe string', () => {
      expect(appClaimCommand.describe).toBeDefined();
    });
  });
});
